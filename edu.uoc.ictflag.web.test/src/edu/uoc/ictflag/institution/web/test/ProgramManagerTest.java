package edu.uoc.ictflag.institution.web.test;
...
public class ProgramManagerTest
{
	...
	private void setupPermissions(IAuthorizationManager authorizationManager, IUserRepository userRepository) throws Exception
	{
		...
		when(authorizationManager.userHasPermission(ui3, "institution", "Program", Permission.EDIT)).thenReturn(true);
		...
		when(authorizationManager.userHasPermission(ui4, "institution", "Program", Permission.EDIT)).thenReturn(true);
	...
	public void getProgramTest() throws Exception
	{
		...
		assertNull(p); // OTHER PROGRAM MANAGER CAN'T ACCESS
		
		p = manager.getProgram("std", 3);
		assertNull(p); // STUDENTS DO NOT HAVE PERMISSION
	}
	...
	public void createOrUpdateProgramTest() throws Exception
	{
		...
		verify(programRepository, times(3)).createOrUpdateProgram(program); // PM1 CAN DO IT 
		
		manager.createOrUpdateProgram("pm2", program);
		verify(programRepository, times(3)).createOrUpdateProgram(program); // PM2 CAN DO IT 
		
		manager.createOrUpdateProgram("std", program);
		verify(programRepository, times(3)).createOrUpdateProgram(program); // STD HAS NO PERMISSION 
	}
}
