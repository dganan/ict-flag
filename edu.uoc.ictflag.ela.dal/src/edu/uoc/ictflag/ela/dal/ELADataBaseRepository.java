...
public abstract class ELADataBaseRepository extends ELABaseRepository
{
	...
	protected Course fixUpCourse(Subject subject, Semester semester) throws Exception
	{
		Course course = new Course();
		
		if (subject != null && semester != null) course.setCode(subject.getCode() + " - " + semester.getCode());
		else course.setCode("unknown");
		
		course.setName(LocalizedString.fromStringFormat("en#" + course.getCode()));
		course.setSubject(subject);
		course.setSemester(semester);
		course.setLanguage("en");
		
		dbHelper.addEntity(course);
